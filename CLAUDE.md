...
- **`Dispatchers.IO` is unavailable in `commonMain`** — iOS has no IO dispatcher. Use `Dispatchers.Default` for all `commonMain` coroutine work, including SQLDelight `asFlow().mapToList/mapToOneOrNull(Dispatchers.Default)` calls.
...
- **Cross-module smart cast**: Kotlin cannot smart-cast a public property declared in a different module, even after a null check. Always capture in a local `val` first: `val ships = shipyard.ships; if (ships != null) { items(ships) { ... } }`.
- **`combine` + single `_localState.update()` timing in tests**: A single `_localState.update {}` schedules a re-emission through `combine` on the test dispatcher but does NOT execute it synchronously. Reading `uiState.value` immediately after `onEvent()` — before `advanceUntilIdle()` — will see the stale value. Always call `testDispatcher.scheduler.advanceUntilIdle()` after each state-changing event before asserting on `uiState.value`. Note: two *consecutive* updates before an advance coalesce into one emission — only the final state propagates — which is why some tests pass without an extra advance even when one seems needed.
- **Fog-of-war with nullable `List<T>` in SQLDelight**: A relational table cannot distinguish "no rows because the API returned null" from "no rows because the list is empty". Add a sentinel column (`ships_cached INTEGER NOT NULL DEFAULT 0`) to the parent table — set `1` when the API returned a non-null list (even if empty), `0` for fog-of-war. The repository's observe flow reads this column to decide whether to map child rows into a list or return `null`.
- **Unscoped transitive dependencies in Hilt**: A repository must be unscoped if *any* of its constructor dependencies are unscoped (session-dependent), even if it has no direct session dep itself. `@Singleton` + an unscoped dep is a silent failure — Hilt compiles it but the singleton captures a stale session reference after logout. Trace constructor deps before adding `@Singleton`.
